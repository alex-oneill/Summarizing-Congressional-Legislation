...
def main():
    pull_full_text = False
    pull_sum_text = False
    version = ''
    while version != "1" and version != '2':
        version = input('Do you want the full text or summary?\n1 = Full Text\n2 = Summary Text\n: ')
    if version == '1':
        pull_full_text = True
    elif version == '2':
        pull_sum_text = True
    ...
    if pull_full_text:
        ...
        session_page = requests.get(f'{billsum_stub}', headers={'Accept': 'application/xml'})
    ...
    if pull_full_text:
        file_name = 'full_text_xml.json'
    else:
        file_name = 'summary_xml.json'

    with open(file_name, 'w') as out_file:
        json.dump(bills_dict, out_file)
...
# todo CHECK WHY SUMMARY IS PULL IS NOT COMPLETING